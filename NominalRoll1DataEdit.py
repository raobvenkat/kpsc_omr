...
class NominalRoll1DataEdit:
    ...
    def __init__(self, root, user_id):
        ...
        self.root.title('Nominal Roll 1 (Descriptive Test) Data Edit')
        try:
    ...
    def load_editfor_values(self):
        try:
            ...
            cur = conn.cursor()
            cur.execute('EXEC usp_NominalRoll1EditFor')
    ...
    def load_data(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                'EXEC usp_LoadNominalRoll1EditGrid @EditFor=?, @UserID=?, @FromID=?, @ToID=?',
    ...
    def update_record(self):
        try:
            ...
            cursor.execute(
                """
                EXEC usp_NominalRoll1EditUpdate
                    @EditFor=?,
                    @UserID=?,
                    @ID=?,
                    @CenterCode=?,
            ...
            self.log_error('NominalRoll1DataEdit', 'Update', ex)
            self.message_var.set(str(ex))
    ...
    def skip_record(self):
        try:
            ...
            self.log_error('NominalRoll1DataEdit', 'Skip', ex)
            self.message_var.set(str(ex))
...
if __name__ == '__main__':
    ...
    try:
        app = NominalRoll1DataEdit(root, 1)
        root.mainloop()
    except KeyboardInterrupt:
