...
REPORTS: List[Dict[str, Any]] = [
    {
        ...
        "proc": "dbo.Sub_CodeAndBookletNoDesc",
    ...
    },
    {
        ...
        "proc": "dbo.ThresholdDesc",
        "update_proc": "dbo.Disc_Update_BubbleIssue",
        "source": "counter_foil",
        "cols": [
            ("ID","ID",50), ("FileName","File",220), ("Barcode","Barcode",120),
            ("Bubble_RegNo","Bubble RegNo",130), ("Min_Bubble_Fill_Pct","Fill %",80),
            ("Final_RegNo","Final RegNo",120), ("IsResolved","Resolved",70),
        ],
        "edit_fields": [
            ("Bubble_RegNo", "Bubble RegNo (corrected)", None),
            ("Final_RegNo",  "Final RegNo (corrected)",  None),
        ],
        "crops": {
            "reg_box": CROP_BUBBLE,
    ...
    },
    {
        ...
        "proc": "dbo.NominalRoll1_Can_Sign_Desc",
    ...
    },
    {
        ...
        "proc": "dbo.NominalRoll1_inv_Sign_Desc",
        "update_proc": "dbo.Disc_Update_NominalRollRow",
        "source": "nominal_roll",
        "cols": [
            ("ID","ID",50), ("Sheet_Type","Type",60), ("FileName","File",200),
...
class DetailPanel(tk.Frame):
    ...
    def _render_detail(self, parent, report: dict, col_names: list, row_values: list):
        ...
        is_nominal = report.get("source") == "nominal_roll"
        if is_nominal and _PIL_OK and fname and os.path.isfile(fname):
            ...
            self._render_nominal_crops(left, fname, report, rd)
            self._render_full_thumb(left, fname)
        elif crops_cfg and _PIL_OK and fname and os.path.isfile(fname):
            left  = tk.Frame(body, bg=C_CARD)
            right = tk.Frame(body, bg=C_CARD)
            left.pack(side="left", fill="both", expand=True, padx=(16, 8), pady=8)
            right.pack(side="right", fill="both", expand=True, padx=(8, 16), pady=8)
            ...
            self._render_full_thumb(left, fname)
        elif _PIL_OK and fname and os.path.isfile(fname):
            left  = tk.Frame(body, bg=C_CARD)
            right = tk.Frame(body, bg=C_CARD)
            left.pack(side="left", fill="both", expand=True, padx=(16, 8), pady=8)
            right.pack(side="right", fill="both", expand=True, padx=(8, 16), pady=8)
    ...
    def _render_crops(self, parent, fname: str, crops_cfg: dict):
        ...
        CROP_LABELS = {
            ...
            "reg_box":  "Register No / Bubble Grid Region",
            "cand_sig": "Candidate Signature Region",
            "inv_sig":  "Invigilator Signature Region",
            "invigilator_nominal": "Invigilator Signature (Nominal Roll)",
        }
        CROP_SIZE = {
            "subject":  (340, 90),
            "booklet":  (340, 110),
    ...
    def _render_nominal_crops(self, parent, fname: str, report: dict, rd: dict):
        """Show nominal-roll sheet thumbnail and invigilator region when applicable."""
        tk.Label(parent, text="Scanned Regions",
                 bg=C_CARD, fg=C_MUTED, font=("Segoe UI", 9, "bold"),
                 anchor="w").pack(fill="x", pady=(0, 4))

        if report["id"] == 11 and _PIL_OK:
            try:
                from core.nominal_roll import get_invigilator_signature_box
                sheet_type = 1 if str(rd.get("Sheet_Type", "Type1")) == "Type1" else 2
                img = Image.open(fname)
                iw, ih = img.size
                box = get_invigilator_signature_box(sheet_type, iw, ih)
                photo = _pil_crop(fname, box, (340, 80))
                if photo:
                    section = tk.Frame(parent, bg="#0d1017",
                                       highlightthickness=1, highlightbackground=C_BORDER)
                    section.pack(fill="x", pady=4)
                    tk.Label(section, text="Invigilator Signature (Nominal Roll)",
                             bg="#0d1017", fg=C_MUTED,
                             font=("Segoe UI", 8, "bold"),
                             anchor="w", padx=6, pady=4).pack(fill="x")
                    self._photo_refs.append(photo)
                    tk.Label(section, image=photo, bg="#0d1017").pack(padx=4, pady=(0, 6))
            except Exception:
                pass

        if report["id"] == 10:
            row_no = rd.get("Row_Number", "")
            tk.Label(parent,
                     text=f"Candidate signature missing on row {row_no}. See full sheet below.",
                     bg=C_CARD, fg=C_MUTED, font=FONT_SMALL,
                     wraplength=340, justify="left").pack(fill="x", pady=4)
    ...
    def _render_detected_values(self, parent, report: dict, rd: dict):
        ...
        for db_col, display_name, _width in report["cols"]:
            ...
            if db_col in ("IsResolved",) and val_str in ("1", "True"):
                ...
                fg = C_RED
            elif db_col == "Min_Bubble_Fill_Pct":
                try:
                    if float(val_str) < 35:
                        fg = C_RED
                except Exception:
