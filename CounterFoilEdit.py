...
class CounterFoilEdit:
    ...
    def log_error(self,error_msg):
        try:
            conn=self.get_connection(); cur=conn.cursor()
            cur.execute('INSERT INTO ErrorLog(ErrorScreen,ErrorModule,ErrorText,ErrorTime) VALUES (?,?,?,GETDATE())', 'CounterFoilEdit', 'CounterFoilEdit', str(error_msg))
            conn.commit(); conn.close()
        except Exception:
            pass
